...
export const Updatepage12 = async (req, res, next) => {
  ...
  if (letsfinish == 0) {
    ...
    if (req.body.page != undefined) {
            const pageId = req.body.page.current.id;
      ...
      if (req.body.page.current.mobiledoc) {
        ...
        if (watermarkPromises && watermarkPromises.length > 0 && watermarkPromises.length == updatedCards.length) {
          try {
            ...
            const url = `https://oemdieselparts.com/ghost/api/admin/pages/${pageId}`;
            const headers = {
            ...
            if (featureImage != "") {
              payload.pages[0].feature_image = featureImage;
            }
            if (payload != "" && typeof payload !== "undefined") {
              const checkFields = async () => {
                try {
                  ...
                  console.log("Feature SUCCESS " + req.body.page.current.id);
                  letsfinish++;
                } catch (error) {
